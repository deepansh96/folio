...
export default class Grid {
  ...
  setMaterial() {
    ...
    this.material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = injectIntoShader(
        injectableVertexShader,
        shader.vertexShader
      );
      shader.fragmentShader = injectIntoShader(
        injectableFragmentShader,
  ...
  calculateGridCurvature() {
    ...
    this.gravityObjects.forEach((object) => {
      ...
      let objectPowerFactor =
        "gridCurvatureProps" in object
  ...
  setupDebug() {
    ...
    if (this.debugFolder != null) {
      ...
      if (this.instance != null && "material" in this.instance) {
        this.debugFolder
          .add(this.debugObject, "uPowerFactor")
          .min(0)
          .max(10)
          .step(0.0001)
          .name("Power Factor")
          .onChange(() => this.calculateGridCurvature());
        this.debugFolder
          .add(this.debugObject, "uDepth")
          .min(0)
          .max(100)
          .step(0.001)
          .name("Depth")
        ...
        this.debugFolder
          ...
          .onChange(() => this.calculateGridCurvature());
        this.debugFolder
          .add(this.debugObject.uWidthOfUV, "value")
          .min(0)
          .max(1)
          .step(0.001)
          .name("uWidthOfUV")
          .onChange(
            () =>
              (this.customUniforms.uWidthOfUV.value = this.debugObject.uWidthOfUV.value)
          );
        this.debugFolder
          .add(this.debugObject.uLocationOfUV, "value")
          .min(0)
          .max(1)
          .step(0.001)
          .name("uLocationOfUV")
          .onChange(
            () =>
              (this.customUniforms.uLocationOfUV.value = this.debugObject.uLocationOfUV.value)
          );
      }
    }
  }
